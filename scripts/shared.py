...
initial_task = 500 # don't change! this was used for the creation of the first batch. 
p16_dir = os.path.join(os.environ.get('nnUNet_raw_data_base'), 'p16')

def getUserspecificEnvironmentVariables(filename):
    env_var_file = open(filename, "r")
    lines = env_var_file.readlines()
    env_var_file.close()

    vars = {}
    for i in lines:
        i_split = i.rstrip('\n').replace('"','').split('=')
        vars[i_split[0]] = i_split[1]
    return vars
